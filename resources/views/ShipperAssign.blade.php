      ...
      <div class="card-body">
        ...
        <form id="trackingForm">
          <div class="mb-3">
            ...
            <input type="text" class="form-control" id="trackingId" placeholder="e.g., TCS12345678" required>
            <div class="invalid-feedback" id="trackingError"></div>
          </div>
...
<script>
  ...
  function submitTracking() {
    ...
    $('#trackingError').hide().text('');
    
    // Validate input
    if (!trackingId) {
      $('#trackingId').addClass('is-invalid');
      $('#trackingError').text('Please enter a tracking ID').show();
      return;
    }
    ...
    if (response.success) {
      Swal.fire({
        ...
        $('#trackingId').val('');
        $('#trackingModal').modal('hide'); // ✅ close the modal
      });
    } else {
      showError(response.message || 'Could not save tracking ID');
    }
  },
      error: function(xhr) {
  ...
  function showError(message) {
    $('#trackingId').addClass('is-invalid');
    $('#trackingError').text(message).show();
